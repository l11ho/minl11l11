...
const InternationalTours = () => {
  ...
  const [travelDate, setTravelDate] = useState(
    queryParams.get('date') || '2026-05-11'
  );
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        ...
        console.error('International tours API error:', err);
  ...
  const getTourName = (tour) => {
    return tour.name || tour.tour_name || tour.title || 'Зарубежный тур';
  };
  ...
  const getTourDuration = (tour) => {
    return tour.duration || tour.trip_duration || tour.days || '5 дней 4 ночи';
  };

  const getTourCurrency = () => {
    return 'USD';
  };
  ...
  const getTourLine = (tour, index) => {
    return (
      ...
      tour.package_type ||
      tour.tour_class ||
      ['Выгодная цена', 'Стандарт', 'Эконом', 'Премиум'][index % 4]
    );
  };
  ...
  const isInternationalTour = (tour) => {
    ...
    if (domesticKeywords.some((keyword) => text.includes(keyword))) {
      return false;
    }
  ...
  const normalizedTours = useMemo(() => {
    return tours.filter(isInternationalTour).map((tour, index) => ({
      raw: tour,
      ...
      <main className="dt-main">
        ...
        {loading ? (
          ...
          <div className="dt-state-card">
            Не найдено зарубежных туров, соответствующих текущему фильтру.
          </div>
        ) : (
